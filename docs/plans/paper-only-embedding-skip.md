# Plan — paper-only mode: skip the embedding pass
...
> on *every* GLOBAL question — on a metered model that is the expensive path, and long contexts
...
| Key | Default | Purpose |
| --- | --- | --- |
| `PAPER_ONLY_MODE` | `false` | Master switch. ⚠ Default `false` so upgrading changes nothing — invariant 5. |
...
| Stuffing without `[seq:N]` labels | — | ⚠ **Silent.** Answers are correct but uncitable | Citation chips vanish | Covered by §12 S2's done-criteria |
| `token_count` undercounts a math-heavy paper | — | Doc classified skippable but overflows in practice | Slow/truncated answers | Lower `PAPER_ONLY_MAX_TOKENS` |
| User asks "show me a figure" on a skipped doc | — | Doc-order fallback returns the first figures, not the relevant ones | Wrong figure surfaced | Known limitation — §11 |
