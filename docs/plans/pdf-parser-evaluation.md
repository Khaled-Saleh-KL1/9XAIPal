# Plan for PDF parser evaluation: add a second extractor, route by document length
...
> **MinerU:** 3.4.4 (current release), installed to `backend/.venv`, run on the **host**, with no
> Docker, no CUDA.
...
| MinerU inline-formula toggle never tested | Not tested | 82 lines of glyph-repair kept unnecessarily | §9 |
