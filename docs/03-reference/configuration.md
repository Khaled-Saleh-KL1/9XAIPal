...
| Key | Default | Purpose |
| --- | --- | --- |
| `OLLAMA_BASE_URL` | `http://localhost:11434` | ⚠ Compose services override this to `http://host.docker.internal:11434`, since inside a container, `localhost` is the container. |
...
| `WHOLE_PAPER_MAX_TOKENS` | `120000` | The ceiling that applies **only when the key above is `True`**: stuff the document when `SUM(chunks.token_count)` is at or below this. ⚠ `token_count` is a `len/4` heuristic that undercounts math and tables, so leave headroom below the model's real window. |
