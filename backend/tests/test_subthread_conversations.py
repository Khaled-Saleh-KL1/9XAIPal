...
All conv_repo calls take user_id explicitly (added by the multi-user auth
retrofit — conversation_turns has no transitive owner to derive it from for
the pure library-wide-chat case, so it's a direct column everywhere).
"""
...
async def _make_user(db_session):
    """A throwaway user row so documents.user_id's FK is satisfied."""
...
async def test_main_chat_vs_subthread_scoping(db_session):
    ...
    u1 = await conv_repo.create_turn(
        db_session, user_id=user_id, conversation_id=conv_id, document_id=doc_id,
    ...
    main = await conv_repo.get_main_chat(db_session, user_id, conv_id)
    assert len(main) == 2
    ...
    # u2, a2, u3
    subtree = await conv_repo.get_thread_subtree(db_session, user_id, u1["id"])
    ...
    # has_children
    assert await conv_repo.has_children(db_session, user_id, u1["id"]) is True
    assert await conv_repo.has_children(db_session, user_id, a1["id"]) is False   # a1 itself has no direct children in this tree
    assert await conv_repo.has_children(db_session, user_id, u2["id"]) is True

    # Message count for the sub-thread
    count = await conv_repo.get_thread_message_count(db_session, user_id, u1["id"])
    ...
    subtree2 = await conv_repo.get_thread_subtree(db_session, user_id, u1["id"])
...
async def test_compaction_is_independent_per_thread(db_session):
    ...
    await db_session.execute(
        text(
            "INSERT INTO documents (id, user_id, filename, original_filename, status) "
            "VALUES (:id, :user_id, 'p.pdf', 'p.pdf', 'complete')"
        ),
        {"id": doc_id, "user_id": user_id},
    )
    await db_session.commit()
    ...
    await conv_repo.create_turn(db_session, user_id=user_id, conversation_id=conv_id, document_id=doc_id, role="user", content="Main q1")
    await conv_repo.create_turn(db_session, user_id=user_id, conversation_id=conv_id, document_id=doc_id, role="user", content="Main q2")

    # Sub-thread activity (rooted at the first main user turn)
    main_turns = await conv_repo.get_main_chat(db_session, user_id, conv_id)
    root = main_turns[0]["id"]

    await conv_repo.create_turn(db_session, user_id=user_id, conversation_id=conv_id, document_id=doc_id, role="user", content="Tangent q1", parent_turn_id=root)
    await conv_repo.create_turn(db_session, user_id=user_id, conversation_id=conv_id, document_id=doc_id, role="user", content="Tangent q2", parent_turn_id=root)
    ...
    main = await conv_repo.get_main_chat(db_session, user_id, conv_id)
    sub = await conv_repo.get_thread_subtree(db_session, user_id, root)
